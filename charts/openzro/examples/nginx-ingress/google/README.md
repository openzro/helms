...
Before starting the setup, refer to the [openZro documentation](https://docs.openzro.io/selfhosted/identity-providers#google-workspace) to configure your Google Identity Provider and generate the necessary parameters:
...
- `customerID`: `xxxxxxx` # The Google Workspace Customer ID belong to the directory of users that will be enumerated for access to openZro. Found [here](https://support.google.com/a/answer/10070793?hl=en).

You will also be required to set up a kubernetes secret for `openzro-gcp-service-account` which contains the `sa.json` belong to the service account generated in the prerequisites.
